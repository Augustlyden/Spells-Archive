...
export async function fetchSpellBook() {
    try {
        wizardLoader?.classList.toggle('hidden');
        const response = await fetch('https://www.dnd5eapi.co/api/2014/spells');
        if (!response.ok) {
            const message = userFriendlyErrors[response.status] || `An unexpected error occurred.`;
...
;
export async function fetchSpellDetails() {
    try {
